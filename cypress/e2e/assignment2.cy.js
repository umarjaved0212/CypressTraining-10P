describe('practice spec', () => {
    ...
    it('Use Fixtures for static data in the signup form', () => {
     ...
     cy.fixture('example.json').then((userData) => {
        ...
        cy.get('#customer_firstname').type(userData.firstname)
        cy.get('#customer_lastname').type(userData.lastname)
        cy.get('#email').clear()
      ...
      it.only('Login as existing user', () => {

        cy.fixture('example.json').then((userData) => {
         cy.visit(userData.url)
         cy.login("email + '@yopmail.com'",userData.password)
